...
```sh
python3 proofs/prove.py              # every proof package, four at a time
python3 proofs/prove.py lru          # one package
python3 proofs/prove.py --list       # the entry points it checks
bend proofs/containers/lru/proof.bend   # or any entry point directly
```

Each package's proof is self-contained in its folder: the specification, the
shadow of the implementation with its invariant, the lemmas and the entry
point `proof.bend`, which states one theorem per public operation. A package
imports only `proofs/lib`, the proofs of the packages its source builds on
(the LRU contains a hash table, the TreeMap a dynamic array), and `src/`.
`prove.py` checks every entry point (every proof file no other package file
imports) and `PROOF.bend`, which gathers them all.

The hash map is proved against an independent association-list
specification (`proofs/containers/hash_table/spec.bend`) for every operation (`new`,
...
(`proofs/containers/lru/spec.bend`: a capacity, a lifetime, the entries oldest first and
...
(`proofs/containers/doubly_linked_list/spec.bend`: the ids in order, the value and
...
(`proofs/containers/balanced_search_tree/spec.bend`: a limit and the entries sorted by key; cursors
