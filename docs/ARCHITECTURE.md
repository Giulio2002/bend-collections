...
| `lru` (benchmarked, `src/lru/fast.bend`) | native `Map<&2, U32>` (encoded key -> slot) plus an arena: `kys: Array<String>`, `ents: Array<Slot<V>>` (`Live{v}` / `Timed{v, deadline}`), `prevs`/`nexts: Array<U32>`, metrics as ten `U32` limbs in one `Array<U32>` | key -> slot through the native Map (patricia trie; the C reference uses a hash table, see docs/C_EQUIVALENCE.md) | intrusive DOUBLY LINKED recency list as arena indices (head = oldest), free list through `nexts`; O(1) touch/evict/remove |
