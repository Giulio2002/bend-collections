...
/* The same flat split-point layout src/fenwick_tree.bend uses: the values
 * live at 2^d + t, and the internal node covering the block [o, o + 2^p)
 * keeps its left half's sum at the block's split point o + 2^(p-1). */
...
/* The Bend driver really builds a one-element fenwick tree, reads it and
...
static uint32_t round_fw(uint32_t op, uint32_t size, uint32_t count,
  ...
  keep(sp);
  switch (op) {
    case 0: for (uint32_t i = 0; i < count; i++) { keep(sp); fw_add(sp, size); } break;
    case 1: for (uint32_t i = 0; i < count; i++) { keep(sp); fw_prefix_op(sp, size); } break;
    case 2: for (uint32_t i = 0; i < count; i++) { keep(sp); fw_range(sp, size); } break;
    case 3: for (uint32_t i = 0; i < count; i++) { keep(sp); fw_len(sp); } break;
    case 4: for (uint32_t i = 0; i < count; i++) { keep(sp); fw_new(sp); } break;
    case 5: for (uint32_t i = 0; i < count; i++) { keep(sp); fw_from(sp); } break;
    default: for (uint32_t i = 0; i < count; i++) { keep(sp); fw_null(sp); } break;
  }
  ...
  for (uint32_t i = 0; i < nulls; i++) { keep(sp); fw_null(sp); }
