...
static inline void uf_new(St *s) {
  s->rng = lcg(s->rng);
  s->chk = mix(s->chk, 1); /* component_count(new(1)) */
}
...
static uint32_t round_uf(uint32_t op, uint32_t size, uint32_t count, uint32_t seed) {
  arena_reset();
  St s;
  uf_init(&s, seed, size);
  ...
  switch (op) {
    case 0: for (uint32_t i = 0; i < count; i++) uf_find(&s, size); break;
    case 1: for (uint32_t i = 0; i < count; i++) uf_union(&s, size); break;
    case 2: for (uint32_t i = 0; i < count; i++) uf_conn(&s, size); break;
    case 3: for (uint32_t i = 0; i < count; i++) uf_size(&s, size); break;
    case 4: for (uint32_t i = 0; i < count; i++) uf_count(&s); break;
    case 5: for (uint32_t i = 0; i < count; i++) uf_new(&s); break;
    default: for (uint32_t i = 0; i < count; i++) uf_null(&s); break;
  }
