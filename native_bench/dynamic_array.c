...
static uint32_t round_da(uint32_t op, uint32_t size, uint32_t count, uint32_t seed) {
  arena_reset();
  St s;
  da_init(&s, seed);
  ...
  da_settle(&s);
  switch (op) {
    case 0: for (uint32_t i = 0; i < count; i++) da_push(&s); break;
    case 1: for (uint32_t i = 0; i < count; i++) da_pop(&s); break;
    case 2: for (uint32_t i = 0; i < count; i++) da_get(&s, size); break;
    case 3: for (uint32_t i = 0; i < count; i++) da_set(&s, size); break;
    case 4: for (uint32_t i = 0; i < count; i++) da_len(&s); break;
    case 5: for (uint32_t i = 0; i < count; i++) da_cap(&s); break;
    case 6: for (uint32_t i = 0; i < count; i++) da_to_list(&s); break;
    case 7: for (uint32_t i = 0; i < count; i++) da_clear(&s); break;
    case 8: for (uint32_t i = 0; i < count; i++) da_reserve(&s, size); break;
    case NULL_OP: for (uint32_t i = 0; i < count; i++) da_null(&s); break;
    default: for (uint32_t i = 0; i < count; i++) da_new(&s); break;
  }
...
int main(int argc, char **argv) {
  ...
  for (uint32_t p = a.reps; p-- > 0;)
    acc_a = mix(acc_a, round_da(a.op, a.size, a.count, a.seed + p));
  uint64_t t1 = now_ns();
  uint64_t t2 = now_ns();
  for (uint32_t p = a.reps; p-- > 0;)
    acc_b = mix(acc_b, round_da(NULL_OP, a.size, a.count, a.seed + p));
  uint64_t t3 = now_ns();
