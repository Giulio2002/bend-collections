/* Optimized C reference for src/deque.bend.
 *
 * Same algorithm and representation: a banker's deque held as two singly
 * linked lists with cached lengths, DQ{flen, front, blen, back}, whose
 * sequence is front ++ reverse(back). Pushes cons onto one side; a pop or
 * peek at an empty side first moves half (floor) of the other side across,
 * reversing the moved part, exactly as ready_front/ready_back do. to_list
 * materialises front ++ reverse(back).
 *
...
static uint32_t round_dq(uint32_t op, uint32_t size, uint32_t count, uint32_t seed) {
  arena_reset();
  St s;
  dq_init(&s, seed);
  ...
  dq_settle(&s);
  switch (op) {
    case 0: for (uint32_t i = 0; i < count; i++) dq_pf(&s); break;
    case 1: for (uint32_t i = 0; i < count; i++) dq_pb(&s); break;
    case 2: for (uint32_t i = 0; i < count; i++) dq_popf(&s); break;
    case 3: for (uint32_t i = 0; i < count; i++) dq_popb(&s); break;
    case 4: for (uint32_t i = 0; i < count; i++) dq_peekf(&s); break;
    case 5: for (uint32_t i = 0; i < count; i++) dq_peekb(&s); break;
    case 6: for (uint32_t i = 0; i < count; i++) dq_len(&s); break;
    case 7: for (uint32_t i = 0; i < count; i++) dq_to_list(&s); break;
    case 8: for (uint32_t i = 0; i < count; i++) dq_new(&s); break;
...
BENCH_MAIN(round_dq, 2048ull * 1024ull * 1024ull)
