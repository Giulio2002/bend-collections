...
def _bitlist(rng):
    lim = rng.choice(['none', 'none', '0', '3', '40', '70'])
    ops = [lim]
    n = 0
    for _ in range(rng.randrange(30, 90)):
        ...
        i = rng.randrange(0, n + 2)
        if c < 0.4:
            ops.append('push:%d' % rng.randrange(2))
            n += 1
        elif c < 0.5:
            ops.append('pop')
            n = max(0, n - 1)
        elif c < 0.62:
            ops.append('set:%d:%d' % (i, rng.randrange(2)))
        elif c < 0.74:
            ops.append('get:%d' % i)
        elif c < 0.8:
            ops.append('count')
