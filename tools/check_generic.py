...
# the spec/math/generic.bend clause each case exercises (integer instances
# U32 and U64 against the proved Nat functions; floats against IEEE)
INT_CLAUSE = {'gcd': 'Gcd.agrees', 'lcm': 'Lcm.checked', 'isqrt': 'Isqrt.agrees', 'iroot': 'Iroot.agrees',
              'ilog': 'Ilog.agrees', 'factorial': 'Factorial.checked', 'perm': 'Perm.checked',
              'comb': 'Comb.checked', 'pow_mod': 'PowMod.agrees', 'mod_inverse': 'ModInverse.agrees',
              'divmod': 'DivMod.agrees', 'bit_length': 'BitLength.agrees', 'clamp': 'Clamp.agrees',
              'gcd_all': 'GcdAll.agrees', 'lcm_all': 'LcmAll.checked', 'prod': 'Prod.checked',
              'sum': 'Sum.checked', 'min': 'Min.agrees', 'max': 'Max.agrees', 'abs': 'Abs.identity',
              'sign': 'Sign.agrees', 'pow': 'Pow.checked'}
F_CLAUSE = {'clamp': 'FClamp.select', 'prod': 'FProd.fold', 'sum': 'FSum.fold', 'min': 'FMin.select',
            'max': 'FMax.select', 'abs': 'FAbs.value', 'sign': 'FSign.select', 'pow': 'FPow.binary'}


def clause(tok):
    ty, op = tok.split(':')[:2]
    return ty.upper() + ' ' + (F_CLAUSE if ty in ('f32', 'f64') else INT_CLAUSE)[op]
...
def rnd_int(rng, w):
    ...
    if c < 0.88: return (1 << w) - 1 - rng.randrange(0, 4)
    if c < 0.94:  # around perfect squares (the square roots correct an estimate)
...
def main():
    ...
    bad = [(clause(t), t, e, g) for (t, e), g in zip(cases, got) if e != g and not (t.startswith(('f32', 'f64')) and nan_word(e) and nan_word(g))]
    covered = {}
    for t, _ in cases:
        covered[clause(t)] = covered.get(clause(t), 0) + 1
    print(json.dumps({'seed': seed, 'cases': len(cases), 'clauses': len(covered), 'failures': len(bad), 'examples': bad[:12]}, indent=1))
