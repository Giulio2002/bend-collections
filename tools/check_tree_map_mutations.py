...
source=(ROOT/'src/containers/balanced_search_tree.bend').read_text()
mutations=[
...
for name,old,new,proof_rejects in mutations:
 ...
 with tempfile.TemporaryDirectory(prefix='bend-tree-map-mutation-') as td:
  tmp=Path(td)
  for folder in ['src','tests/support','tests/tree_map']:
  ...
  shutil.copy(ROOT/'proofs/TREE_MAP_COMPONENT_PROOF.bend',tmp/'proofs/TREE_MAP_COMPONENT_PROOF.bend')
  ...
  (tmp/'src/containers/balanced_search_tree.bend').write_text(mutated)
  built=subprocess.run([BEND,'tests/tree_map/main.bend','-o','build/tree-map/test'],cwd=tmp,text=True,capture_output=True,timeout=60, env=ENV)
  ...
  proof=subprocess.run([BEND,'proofs/TREE_MAP_COMPONENT_PROOF.bend'],cwd=tmp,text=True,capture_output=True,timeout=60, env=ENV)
