...
# the spec/math/f64.bend clause each operation's cases exercise
CLAUSE = {'add': 'Add.value', 'sub': 'Sub.value', 'mul': 'Mul.value', 'div': 'Div.value',
          'sqrt': 'Sqrt.value', 'lt': 'Lt.value', 'le': 'Le.value', 'eq': 'Eq.value', 'neg': 'Neg.value',
          'abs': 'Abs.value', 'copysign': 'Copysign.value', 'of_nat': 'OfNat.value'}
...
def tie_pair(rng):
    # short significands: 27-bit products and 53/54-bit sums often land exactly
    # halfway, near 1 and deep in the subnormal range
    k = rng.choice([0, 0, -1060, -1030, 900, rng.randrange(-1070, 1000)])
    m1, m2 = rng.randrange(1, 1 << 27) | 1, rng.randrange(1, 1 << 27) | 1
    s1, s2 = rng.choice([1.0, -1.0]), rng.choice([1.0, -1.0])
    if rng.random() < 0.5:
        return bits(s1 * math.ldexp(m1, k // 2 - 26)), bits(s2 * math.ldexp(m2, k - k // 2 - 26))
    m3 = rng.randrange(1 << 52, 1 << 53) | 1
    return bits(s1 * math.ldexp(m3, k - 52)), bits(s2 * math.ldexp(1, k - 53 - rng.randrange(0, 3)))
...
def main():
    ...
    bad = [(CLAUSE[t.split(':')[0]], t, e, g) for (t, e), g in zip(cases, got) if not (e == g or (e == 'NaN' and is_nan(g)))]
