...
with tempfile.TemporaryDirectory(prefix='owned-array-') as directory:
    ...
    control=subprocess.run(cmd,capture_output=True,text=True,timeout=30, env=ENV)
    ...
    bad=subprocess.run(cmd,capture_output=True,text=True,timeout=30, env=ENV)
    ...
    bad=subprocess.run([BEND,str(negative)],capture_output=True,text=True,timeout=30, env=ENV)
