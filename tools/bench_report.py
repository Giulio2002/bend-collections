...
def main():
    ...
    if over or bad:
        ...
        out.append('and %d workloads could not be measured at all. The nine `lru.*`'
                   % len(bad))
        out.append('operations the contract requires have **no rows in this frozen table**:')
        out.append('the table lives in the protected `benchmarks/workloads.py`. The')
        out.append('benchmarked cache is `src/lru/fast.bend` (native, proven:')
        out.append('`proofs/lru_fast.bend`), with its optimized C reference')
        out.append('`benchmarks/native/lru.c`; its rows are proposed in')
        out.append('`docs/BENCHMARK_CHANGE_PROPOSAL.md` and measured with this runner\'s own')
        out.append('machinery by `python3 tools/lru_measure.py` (supplemental report')
        out.append('`build/performance/lru_experiment.json`, table in')
        out.append('`docs/C_EQUIVALENCE.md`). The keyed `lru.*` rows are far over 2.5x there')
        out.append('as well; the native crit-bit `Map` bounds them. (The retained cache')
        out.append('itself cannot be compiled natively: `docs/VALIDATION.md`, "arity over 255".)')
        out.append('')
