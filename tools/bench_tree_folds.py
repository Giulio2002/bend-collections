...
bin=ROOT/'build/tree-fold/after';before=ROOT/'build/tree-fold/before';ref=b.REFBIN/'balanced_search_tree';log=[]
...
for row in selected:
 try:
  count=({(7,64):768000,(7,4096):20400,(7,131072):2080,(8,64):204000,(8,4096):3400,(8,131072):55}[(row['op'],row['size'])])*3;reps=1;samples=[]
  log.append(f'Count uses 3x previously calibrated traversal count: {count}; every sample must meet existing timing floors.')
 ...
 (ROOT/'build/tree-fold/benchmarks.json').write_text(json.dumps(report,indent=2)+'\n')
 (ROOT/'build/tree-fold/calibration.log').write_text('\n'.join(log)+'\n')
